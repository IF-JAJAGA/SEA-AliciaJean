...
void create_process (func_t f, void* args, unsigned int stack_size ){
	...
	if ( first_process != NULL ){
		...
		//Création du first_process qui boucle sur lui-même puisqu'il est seul
		pcb->nextProcess = pcb;
		first_process = pcb;
	}
}
...
void start_current_process() {
	//lancement de la fonction lors de son premier appel
...
void elect (){
	while(current_process -> nextProcess -> state == TERMINATED){
		...
		if (dead_process == first_process){
			//Réhabilitation du first_process dans le cas où celui-ci va être supprimé
			first_process = first_process->nextProcess;
		}

		if (dead_process == current_process && current_process->state == TERMINATED){
			//Retour vers le process_idle lorsqu'il n'y a plus qu'un seul processus et que celui-ci est en état TERMINATED
...
void start_sched(){
	//Création du process_idle pour initialiser la chaîne de processus
	process_idle = phyAlloc_alloc(sizeof(struct pcb_s));
	init_pcb(process_idle, NULL, idle, STACK_SIZE);
